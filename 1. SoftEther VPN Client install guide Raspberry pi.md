...
__사무실에서 기본 설정을 마친 후 현장 설치 시 /etc/hosts 파일에 vpnserver의 IP만 변경 혹은 추가해 주면 됩니다__
...
1. vpnclient 실행 후 중지 (config 파일 생성), /usr/local/vpnclient/vpn_client.config파일에서 AllowRemoteConfig false 를 true로 변경 (VPN 원격 관리 허용)
<pre>
...
/etc/dhcp/dhclient.conf 파일에 #timeout 60; 주석해제 후 값 변경
<pre>
...
1. eth1 static ip 설정  __(인터페이스 설정)__
    ...
    /etc/dhcpcd.conf 파일 하단에 다음 내용 추가
<pre>
<code>interface eth1
...
2. /etc/iptables-hs에 MASQUERADE추가
