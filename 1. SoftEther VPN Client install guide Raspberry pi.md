...
서버 주소: vpnserver:443 (192.168.0.6)

가상 랜카드 명: soft (실제 인터페이스엔 vpn_soft 라고 표시됨)

__DNSMASQ__

DHCP 게이트 웨이: 172.26.1.1 (eth1 사용)

DHCP 서브넷 마스크: 172.26.1.0/30

DHCP IP 임대 범위: 172.26.1.2 - 172.26.1.2 (12시간, 카드리더기 ip = 172.26.1.2)

__SYSTEM__

ipv4.ip_forward=1 설정 

MASQUERADE 설정 (VPN Client가 외부로 나갈 때 사용, /etc/iptables-hs)